...
<div class="card">
    <div class="card-header">
        <h3 class="page-title d-inline">@lang('labels.backend.organization_departments.title')</h3>
        @if(auth()->user()->isAdmin() || (auth()->user()->is_admin == 1 && $officeLocation == 0))
        <div class="float-right">
            <a href="{{ route('admin.organization_departments.create') }}"
                ...
                <div class="table-responsive">
                    <div class="d-block">
                        <ul class="list-inline">
                            ...
                            |
                            <li class="list-inline-item">
                                <a href="{{ route('admin.organization_departments.index') }}?show_deleted=1"
                    ...
                    <table id="myTable" class="table table-bordered table-striped @if(auth()->user()->isAdmin()) @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('labels.backend.organization_departments.fields.department_id')</th>
                                <th>@lang('labels.backend.organization_departments.fields.department_name')</th> 	
                                @if( request('show_deleted') == 1 )
...
<script>
    ...
    $(document).ready(function () {
        var route = '{{route('admin.organization_departments.get_data')}}';

        @if(request('show_deleted') == 1)
            route = '{{route('admin.organization_departments.get_data',['show_deleted' => 1])}}';
        @endif
