...
<div class="card">
    <div class="card-header">
        <h3 class="page-title d-inline">@lang('labels.backend.officeorder_cm.title')</h3>
        @if(auth()->user()->isAdmin() || (auth()->user()->is_admin == 1 && $officeLocation == 0))
        <div class="float-right">
            <a href="{{ route('admin.officeorder_content_management.create') }}"
                ...
                <div class="table-responsive">
                    <div class="d-block">
                        <ul class="list-inline">
                            ...
                            |
                            <li class="list-inline-item">
                                <a href="{{ route('admin.officeorder_content_management.index') }}?show_deleted=1"
                    ...
                    <table id="myTable" class="table table-bordered table-striped @if(auth()->user()->isAdmin()) @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('labels.backend.officeorder_cm.fields.title')</th>
                                @if( request('show_deleted') == 1 )
...
<script>
    ...
    $(document).ready(function () {
        var route = '{{route('admin.officeorder_content_management.get_data')}}';

        @if(request('show_deleted') == 1)
            route = '{{route('admin.officeorder_content_management.get_data',['show_deleted' => 1])}}';
        @endif
