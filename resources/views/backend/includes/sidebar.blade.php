...
<div class="sidebar">
    ...
    $id = Auth::user()->id;
     ...
     $department = array();
     $logo = '';
    $studentdetail = DB::table('user_details')->where('user_id', $id)->first();
    if (!empty($studentdetail)) {
        $department_id = $studentdetail->department_id;
           $department = DB::table('departments')->where('id', $department_id)->first();

        $logo = $department->logo;
        $userDeptRole = $studentdetail->organisationaldept_role;
        $deptRole = DB::table('department_role')->where('id', $userDeptRole)->first();
        $departmentRole = $deptRole->parent_id;
        ?>
...
<?php } ?>
   ...
   <a class="navbar-brand" href="{{ route('admin.dashboard') }}">
        @if(isset($logo))
        <img class="navbar-brand-full" src="{{asset('assets/images/'.$logo)}}" alt="Square Logo">
        @else
        <img class="navbar-brand-full" src="{{asset('storage/logos/'.config('logo_popup'))}}"  alt="Square Logo">
        @endif
        <img class="navbar-brand-minimized" src="{{asset('storage/logos/'.config('logo_popup'))}}"  alt="Square Logo">
        </a>
          </div>
...
<li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/courses*','user/lessons*','user/tests*','user/questions*']), 'open') }}">
            ...
            <li class="nav-item">
                <a class="nav-link {{ active_class(Active::checkUriPattern('admin/dashboard')) }}"
                   href="{{ route('admin.elitedashboard.index') }}">
                    <i class="fa fa-dashboard"></i>  @lang('menus.backend.sidebar.elitedashboard')
                </a>
            </li> 
            ...
            <li class="divider"></li>
                ...
                <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern('admin/*'), 'open') }}">
                    ...
                    <ul class="nav-dropdown-items sub-menu">
						...
						<li class="nav-item">
                            <a class="nav-link {{ active_class(Active::checkUriPattern('admin/contactus')) }}"
                               href="{{ route('admin.feedback') }}"> <i class="fa fa-info-circle"></i>
                                Feedback
