...
@section('content')
{{ html()->form('POST', route('admin.newsflash.store'))->acceptsFiles()->class('form-horizontal')->open() }}
<div class="card">
    <div class="card-header">
        <h3 class="page-title d-inline">@lang('labels.backend.news.create')</h3>
            ...
            <div class="col-12">
                <div class="form-group row"> 
                    <div class="col-12 col-lg-6 form-group">
                        ...
                        {!! Form::select('ministry_id[]', $ministry,  (request('ministry_id')) ? request('ministry_id') : old('ministry_id'), ['class' => 'form-control select2', 'multiple' => true, 'required']) !!}
                    </div>
                    <div class="col-12 col-lg-6 form-group">
                        {!! Form::label('department_id', trans('labels.backend.departments.fields.department_name'), ['class' => 'control-label require']) !!}
                        {!! Form::select('department_id[]', $departments,  (request('department_id')) ? request('department_id') : old('department_id'), ['class' => 'form-control select2', 'multiple' => true, 'required']) !!}
                    </div>
                ...
                <div class="form-group row"> 
                    ...
                    <div class="col-12 col-lg-6 form-group">
                        ...
                        {!! Form::text('start_date', old('start_date'), ['class' => 'form-control date','pattern' => '(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))', 'placeholder' => trans('labels.backend.news.fields.start_date').' (Ex . 2019-01-01)', 'autocomplete' => 'off', 'required']) !!}
                    </div>
                    <div class="col-12 col-lg-6 form-group">
                        {!! Form::label('end_date', trans('labels.backend.news.fields.end_date').' (yyyy-mm-dd)', ['class' => 'control-label require']) !!}
                        {!! Form::text('end_date', old('end_date'), ['class' => 'form-control date','pattern' => '(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))', 'placeholder' => trans('labels.backend.news.fields.end_date').' (Ex . 2019-01-01)', 'autocomplete' => 'off', 'required']) !!}
                    </div>
