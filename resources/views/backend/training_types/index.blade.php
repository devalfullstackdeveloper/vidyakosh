...
<div class="card">
    <div class="card-header">
        <h3 class="page-title d-inline">@lang('labels.backend.training-type.title')</h3>
        @if(auth()->user()->isAdmin() || (auth()->user()->is_admin == 1 && $officeLocation == 0))
        <div class="float-right">
            <a href="{{ route('admin.training_types.create') }}"
                    ...
                    <table id="myTable" class="table table-bordered table-striped @if(auth()->user()->isAdmin()) @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('labels.backend.training-type.fields.department_id')</th>
                                <th>Title</th>
								<th>Status</th>
                                @if( request('show_deleted') == 1 )
...
<script>
    ...
    $(document).ready(function () {
        var route = '{{route('admin.training_types.get_data')}}';
        @if(request('show_deleted') == 1)
            route = '{{route('admin.training_types.get_data',['show_deleted' => 1])}}';
        @endif
        ...
        $('#myTable').DataTable({
            ...
            columns: [
                ...
                {data: "training_types_name", name: 'training_types_name'}, 
