...
class ApiController extends Controller
{
    ...
    public function storeTestResults(Request $request){
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Successfully Test Results added !'
        ], 201);
    }
    
     public function storeTestResultsAnswer(Request $request){
        $questionDetails = array();
        $optionDetails = array();
        $testResultDetails = array();
        if(isset($request->question_id)){
            $questionDetails = Question::find($request->question_id);
        } 
        if(isset($request->test_result_id)){
            $testResultDetails = TestsResult::find($request->test_result_id);
        } 
        $testResult = new TestsResultsAnswer();
        $testResult->save();
        $testResultDetails->question()->attach($testResult);
        $testResultDetails->save();
        return response()->json([
            'status' => 'success',
            'message' => 'Successfully Test Results added !'
        ], 201);
