...
public function crts(){
    ...
    public function purchasedCoursesFilter($course_year){
        $orders = Order::where('status','=',1)
            ->where('user_id','=',$this->id)
            ->pluck('id');
        $courses_id = OrderItem::whereIn('order_id',$orders)
            ->where('item_type','=',"App\Models\Course")
            ->pluck('item_id');
        $courses = Course::whereIn('id',$courses_id)->whereYear('created_at',$course_year)
            ->get();
        return $courses;
    }
    ...
    public function CompletedCourses(){
        ...
        $courses_id = OrderItem::whereIn('order_id',$orders)
            ->where('item_type','=',"App\Models\Course")
            ->pluck('item_id');
        $courses_ids = ChapterStudent::where('user_id','=',$this->id)
            ->whereIn('course_id',$courses_id)
            ->pluck('course_id');
        $courses = Course::whereIn('id',$courses_ids)
            ->get();
        return $courses;
    }
