...
class ScormReader
{
    public function SequenceReader($domManifest, $PK, $postdoc, $mode)
    {
		...
		if ($this->GetRequestType($post)=="Initialize")
		{
		...
		$GET_SCO = $GLOBALS['db']->Execute("select * from content where identifier='" . trim($this->sco($post)) . "' and scormtype='sco' and  lessons_ID='".$PK."'");			
		
		$init=array();
		$init["content_ID"]=$GET_SCO->fields["id"];
		$init["users_LOGIN"]=$_SESSION["learnerID"];
		ActiveinsertOrupdateTableData("scorm_data_2004",$init," content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."' ");
		
		$GET_COCD = $GLOBALS['db']->Execute("select * from scorm_data_2004 where content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."'");			

	 	// echo "select * from scorm_data_2004 where content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."'";
		if($GET_COCD->fields["cocd"]!=NULL or $GET_COCD->fields["cocd"]!="" and !$GET_COCD->EOF)
		{
		$f = $dom->createDocumentFragment();
	 
        $f->appendXML($GET_COCD->fields["cocd"]);
        $cocd=$dom->documentElement->appendChild($f);
		//echo $GET_COCD->fields["cocd"];
 		
		$COCD_DATA=array();
		$COCD_DATA["content_ID"]=$GET_SCO->fields["id"];
		$COCD_DATA["users_LOGIN"]=$_SESSION["learnerID"];
		$COCD_DATA["timestamp"]=$cocd->getElementsByTagName('sessionTime')->item(0)->nodeValue;
		$COCD_DATA["lesson_location"]=$cocd->getElementsByTagName('location')->item(0)->nodeValue;
		$COCD_DATA["maxtimeallowed"]="";
		$COCD_DATA["timelimitaction"]=$cocd->getElementsByTagName('timeLimitAction')->item(0)->nodeValue;
		$COCD_DATA["masteryscore"]="";
		$COCD_DATA["datafromlms"]="";
		$COCD_DATA["entry"]=$cocd->getElementsByTagName('entry')->item(0)->nodeValue;
		$COCD_DATA["total_time"]=$cocd->getElementsByTagName('totalTime')->item(0)->nodeValue;
		$COCD_DATA["comments"]="";
		$COCD_DATA["comments_from_lms"]=$cocd->getElementsByTagName('commentsFromLMS')->item(0)->nodeValue;
		$COCD_DATA["success_status"]=$cocd->getElementsByTagName('successStatus')->item(0)->nodeValue;
		$COCD_DATA["score"]=$cocd->getElementsByTagName('score')->item(0)->nodeValue;
		$COCD_DATA["scorm_exit"]=$cocd->getElementsByTagName('exit')->item(0)->nodeValue;
		$COCD_DATA["minscore"]=$cocd->getElementsByTagName('score')->item(0)->getElementsByTagName('min')->item(0)->nodeValue;
		$COCD_DATA["maxscore"]=$cocd->getElementsByTagName('score')->item(0)->getElementsByTagName('max')->item(0)->nodeValue;
		$COCD_DATA["score_scaled"]=$cocd->getElementsByTagName('score')->item(0)->getElementsByTagName('scaled')->item(0)->nodeValue;
		$COCD_DATA["suspend_data"]=$cocd->getElementsByTagName('suspendData')->item(0)->nodeValue;
		$COCD_DATA["completion_status"]=$cocd->getElementsByTagName('completionStatus')->item(0)->nodeValue;
		 
		$COCD_DATA["lesson_status"]=$cocd->getElementsByTagName('lessonStatus')->item(0)->nodeValue;
		 
		
		
		
		ActiveinsertOrupdateTableData("scorm_data_2004",$COCD_DATA," content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."' ");
		...
		if ($this->GetRequestType($post)=="Commit")
		{
		$GET_SCO = $GLOBALS['db']->Execute("select * from content where identifier='" . trim($this->sco($post)) . "' and scormtype='sco' and  lessons_ID='".$PK."'");			
		$init=array();
		$init["content_ID"]=$GET_SCO->fields["id"];
		$init["users_LOGIN"]=$_SESSION["learnerID"];
		$init["cocd"]=trim($this->createCOCD($post));
		ActiveinsertOrupdateTableData("scorm_data_2004",$init," content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."' ");
		$cocd=$post->getElementsByTagName('cocd')->item(0);
		$COCD_DATA=array();
		$COCD_DATA["content_ID"]=$GET_SCO->fields["id"];
		$COCD_DATA["users_LOGIN"]=$_SESSION["learnerID"];
		$COCD_DATA["timestamp"]=$cocd->getElementsByTagName('sessionTime')->item(0)->nodeValue;
		...
		ActiveinsertOrupdateTableData("scorm_data_2004",$COCD_DATA," content_ID='" . $GET_SCO->fields["id"] . "' and users_LOGIN='".$_SESSION["learnerID"]."' ");
			...
			if ($this->GetRequestType($post)=="Terminate")
		{
		$GET_SCO = $GLOBALS['db']->Execute("select * from content where identifier='" . trim($this->sco($post)) . "' and scormtype='sco' and  lessons_ID='".$PK."'");			
		$init=array();
		$init["content_ID"]=$GET_SCO->fields["id"];
